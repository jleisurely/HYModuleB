...
#import "HYConnectorA.h"
#import "HYModuleADetailViewController.h"
#import "HYModuleAXXXItem.h"
 ...
 */
@interface HYConnectorA ()<HYModuleServicePrt> {
...
+ (void)load {
    ...
    @autoreleasepool {
        ...
        [HYComponentManager registerConnector:[self getInstance]];
    }
}
 ...
 *  单例
 */
+ (nonnull HYConnectorA *)getInstance {
    
    static HYConnectorA *sharedConnector = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        sharedConnector = [[HYConnectorA alloc] init];
    });
...
- (nullable UIViewController *)connectToOpenURL:(nonnull NSURL *)URL params:(nullable NSDictionary *)params {
    ...
    if ([self canOpenURL:URL]) {
        ...
        HYModuleADetailViewController *viewController = [[HYModuleADetailViewController alloc] init];
        if (params[@"key"] != nil) {
...
- (nullable id)connectToHandleProtocol:(nonnull Protocol *)servicePrt {
    ...
    if (servicePrt == @protocol(HYModuleServicePrt)) {
...
- (nonnull id<HYModuleItemPrt>)getItemWithName:(nonnull NSString *)name
                                               age:(int)age {
    HYModuleAXXXItem * item = [[HYModuleAXXXItem alloc] initWithItemName:name itemAge:age];
    return item;
}

// 服务接口3: 外部组件给组件A传入一个协议化对象
- (void)deliveAprotocolModel:(nonnull id<HYModuleItemPrt>)item {
