...
static NSMutableDictionary<NSString *, id<HYCMConnectorPrt>> *gConnectorMap = nil;

@implementation HYComponentManager

#pragma mark - 向总控制中心注册挂接点

+ (void)registerConnector:(nonnull id<HYCMConnectorPrt>)connector {
    
    if (![connector conformsToProtocol:@protocol(HYCMConnectorPrt)]) {
        return;
    }
...
+ (BOOL)canRouteURL:(nonnull NSURL *)URL {
    ...
    // 遍历connector不能并发
    [gConnectorMap enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString * _Nonnull key, id<HYCMConnectorPrt>  _Nonnull connector, BOOL * _Nonnull stop) {
...
+ (BOOL)routeURL:(nonnull NSURL *)URL withParameters:(nullable NSDictionary *)params {
    ...
    [gConnectorMap enumerateKeysAndObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSString * _Nonnull key, id<HYCMConnectorPrt>  _Nonnull connector, BOOL * _Nonnull stop) {
        ...
        if([connector respondsToSelector:@selector(connectToOpenURL:params:)]){
            ...
            if(returnObj && [returnObj isKindOfClass:[UIViewController class]]){
                ...
                if ([returnObj isKindOfClass:[HYCMTipViewController class]]) {
                    
                    HYCMTipViewController *tipController = (HYCMTipViewController *)returnObj;
                    ...
                    [[HYNavigator getInstance] hookShowURLController:returnObj baseViewController:params[kLJRouteViewControllerKey] routeMode:params[kLJRouteModeKey]?[params[kLJRouteModeKey] intValue]:ELJNavigationModePush];
                    success = YES;
                }
    ...
    if (!success && queryCount == gConnectorMap.count) {
        ...
        [((HYCMTipViewController *)[UIViewController lj_notFound]) showDebugTipController:URL withParameters:params];
        return NO;
    }
#endif
...
+ (nullable UIViewController *)viewControllerForURL:(nonnull NSURL *)URL withParameters:(nullable NSDictionary *)params {
    ...
    if (returnObj) {
        if ([returnObj isKindOfClass:[HYCMTipViewController class]]) {
#if DEBUG
            [((HYCMTipViewController *)returnObj) showDebugTipController:URL withParameters:params];
#endif
            return nil;
        } else if([returnObj class] == [UIViewController class]){
            return nil;
