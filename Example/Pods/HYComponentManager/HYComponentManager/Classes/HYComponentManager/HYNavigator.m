...
@interface HYNavigator () {
    
    BOOL (^_routeBlock)(UIViewController * controller, UIViewController * baseViewController, ELJNavigationMode routeMode);
}

@end

@implementation HYNavigator

+ (HYNavigator *)getInstance {
    
    static HYNavigator *sharedConnector = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        if (sharedConnector == nil) {
            
            sharedConnector = [[HYNavigator alloc] init];
        }
    });
 ...
 * 设置通用的拦截跳转方式；
 */
- (void)setHookRouteBlock:(BOOL (^)(UIViewController *controller, UIViewController *baseViewController, ELJNavigationMode routeMode))routeBlock {
...
@implementation HYNavigator (HookRouteBlock)
