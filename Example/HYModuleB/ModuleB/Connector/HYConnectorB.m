 ...
 */
@interface HYConnectorB ()<HYModuleServicePrt,HYCMConnectorPrt> {
 ...
 *  单例
 */
+ (nonnull HYConnectorB *)getInstance {
    
    static HYConnectorB *sharedConnector = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        sharedConnector = [[HYConnectorB alloc] init];
    });
...
- (nullable UIViewController *)connectToOpenURL:(nonnull NSURL *)URL params:(nullable NSDictionary *)params {
    ...
    if ([self canOpenURL:URL]) {
        ...
        HYModuleBDetailViewController *viewController = [[HYModuleBDetailViewController alloc] init];
        if (params[@"key"] != nil) {
...
// 服务接口2: 组件B给外部组件提供一个协议化对象
- (nonnull id)getItemWithName:(nonnull NSString *)name
                                               age:(int)age {
    
    HYModuleBXXXItem * item = [[HYModuleBXXXItem alloc] initWithItemName:name itemAge:age];
    
    return item;
}

// 服务接口3: 外部组件给组件A传入一个协议化对象
- (void)deliveAprotocolModel:(nonnull id)item {
